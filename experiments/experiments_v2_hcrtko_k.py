...
output_dir  =  "/Users/qgf169/Documents/python/Usleep/results/HCRTKO_K/"
groups      = ['kornum-HCRTKO']
splits      = ["TRAIN","TEST"]
    ...
    for i in range(len(splits)): # across datasplits 
        ...
        for j in range(len(aid)): # across animals 
            if aid[j]!='12':
                ...
                if (aid[j]!='M39'): 
                    sig  = data[groups[k]][splits[i]][aid[j]]["PSG"]['EEG2'][:]
                else:
                    sig  = data[groups[k]][splits[i]][aid[j]]["PSG"]['EEG1'][:]
