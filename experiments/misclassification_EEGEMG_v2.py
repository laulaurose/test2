...
def post_pred_dis(pred_eeg,pred_emg,n_classes):
    ...
    for i in range(ints): # rows
        for j in range(ints): # columns
...
def  plot_hypnodensity(hyp_EEG,hyp_EMG,ss_joint,xEEG,xEMG,aid,outdir,ss_int,hyp): 
    
    xEEG = xEEG.reshape(xEEG.shape[0]*xEEG.shape[1],xEEG.shape[2],xEEG.shape[3])
    xEMG = xEMG.reshape(xEMG.shape[0]*xEMG.shape[1],xEMG.shape[2],xEMG.shape[3])

    df_EEG         = pd.DataFrame(hyp_EEG)
    df_EMG         = pd.DataFrame(hyp_EMG)
    df_hyp         = pd.DataFrame(hyp)
    df_hyp.columns = ["ss"]
    df_EEG.columns = ["WAKE","NREM","REM"]
    df_EMG.columns = ["WAKE","NREM","REM"]

    ss = pd.DataFrame(np.where(ss_joint==ss_int,1,0))
    ss.columns = ["ss"]
    ss['bout'] = (ss.ss!=ss.ss.shift(1)).cumsum() 
    ss = ss.merge(ss.groupby('bout').ss.count().reset_index().rename(columns={'ss':'len_bout'}), how='left')   
    ...
    for j in range(len(idx)):
        st_id = idx[j]-15 # one minute back 
        sl_id = idx[j]+15 # one minute forward 

        if (st_id>0) & (sl_id<len(xEEG)-15): 
            plot_df_EEG     = df_EEG.iloc[st_id:sl_id].reset_index()
            plot_df_EMG     = df_EMG.iloc[st_id:sl_id].reset_index()
            plot_df_hyp     = df_hyp.iloc[st_id:sl_id].reset_index()
            ...
            repeated_hyp    = pd.DataFrame(np.repeat(plot_df_hyp.values, 4, axis=0))
            width_cm = 15.49
            height_cm = 10.3
            dpi = 600
            ...
            fig, ax= plt.subplots(5,1,figsize=(width_in_inches,height_in_inches),sharex=True)#, sharex=True)
            
            ax[0].plot(t_hypnodensity,repeated_hyp,color="black")
            ax[0].set_xlim(0,120)
            ax[0].set_ylim(-0.1,1.1)
            ax[0].set_ylabel("Hyp")
            ax[0].set_yticks([0, 1, 2,3, 4])
            ax[0].set_yticklabels(["W", "N", "R","DA", "C"])
            handles = []
            labels = []
            df_hypnodensity = repeated_df_EEG.iloc[:,1:4]
            ys = np.zeros(t_hypnodensity.shape)
            for i, col in enumerate(df_hypnodensity.columns):
                bar = ax[1].bar(t_hypnodensity, df_hypnodensity[col], label=col, width=t_hypnodensity[1]-t_hypnodensity[0], bottom=ys,color=col_[i])
            ...
            ys = np.zeros(t_hypnodensity.shape)
            for i, col in enumerate(df_hypnodensity.columns):
                bar = ax[2].bar(t_hypnodensity, df_hypnodensity[col], label=col, width=t_hypnodensity[1]-t_hypnodensity[0], bottom=ys,color=col_[i])
                ys +=df_hypnodensity[col].astype('float64')
                handles.append(bar[0])
                labels.append(col)
            ax[2].set_xlim(0,120)
            ...
            ax[3].pcolormesh(t, f,np.abs(Z), shading='gouraud')
            ax[3].set_ylabel('EEG')

           
            ax[4].plot(time_sig,nEMG)
            ax[4].set_ylabel("EMG")
            ax[4].set_xlabel("Time [s]")
            ax[4].set_ylim(-10,10)

            for axis in ax:
                axis.yaxis.set_label_coords(-0.07, 0.5)  # Adjust -0.1 to move labels further left or right as needed
            
            fig.legend(handles, labels, loc='upper center', bbox_to_anchor=(0.5, 0.97), ncol=len(labels))
            plt.savefig(directory+ "hyp_EEG_"+str(ss_int) + "_" +str(j)+ ".png",dpi=600)
            plt.close()
...
output_dir  =  "/Users/qgf169/Documents/python/Usleep/results/kornum/"
    ...
    for i in range(len(splits)): # across datasplits 
        ...
        for j in range(len(aid)): # across animals 
            if aid[j]!='12':
                ...
                sig  = data[groups[k]][splits[i]][aid[j]]["PSG"][elec[1]][:]
                ...
                # plot hypnodensity (EEG model)
                joint_ss = post_pred_dis(hyp,pred_EEG.reshape(-1, 3).argmax(axis=1),3)
                plot_hypnodensity(pred_EEG.reshape(-1, 3),pred_EMG.reshape(-1, 3),joint_ss,x_eeg,x_emg,aid[j],output_dir,0,hyp) # Wake - REM 


                 # plot 
                # folder_path="/Users/qgf169/Documents/python/Usleep/results/kornum/"+aid[j] + "/"
                ...
                # axs[0].set_title("Power EEG")
                # freqs, psd = signal.welch(emg, fs)
                # axs[1].plot(freqs,psd)
                # axs[1].set_title("Power EMG")
                # plt.savefig(folder_path+"psd.png")
                # plt.close()

                # fig, axs = plt.subplots(2, 1, figsize=(10, 8))
                # t = np.arange(len(sig)) / fs
                # axs[0].plot(t[0:128*60*60],sig[0:128*60*60])
                # axs[0].set_ylabel("EEG")
                # axs[1].plot(t[0:128*60*60],emg[0:128*60*60])
                # axs[1].set_ylabel("EMG")
                # plt.savefig(folder_path+"signals.png")
                # plt.close()
