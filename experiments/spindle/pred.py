...
def plot_cm(cm,normalize=False,
            cmap="Blues",title=None,out_path=None):
    ...
    for i in range(normalized_cm.shape[0]):
        for j in range(normalized_cm.shape[1]):
            ax.text(j, i, f'{normalized_cm[i, j]:.2f}%',
                 ha="center", va="center",
...
for k in range(len(groups)): # across groups
        ...
        for j in range(len(aid)): # across animals 
            ...
            if (E==2) & (modeltype=="all"):
                ...
                for e in range(len(elecs)):
                    ...
                    sig  = data[groups[k]][splits[i]][aid[j]]["PSG"][elecs[e]][:]
                    n = len(sig) // 512
    ...
    plot_cm(cm_all,normalize=False,cmap="Blues",title=groups[k],out_path=output_dir)
